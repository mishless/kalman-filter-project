...
class MLKalmanAssociation:
    ...
    def associate(self, measurements):
        ...
        nu = np.zeros((n, 4))
        for i, (mx, my, mw, mh) in enumerate(measurements):
            new_z = np.array([mx - self.mu_bar[0], my -
                              self.mu_bar[2], mw - self.mu_bar[4], mh - self.mu_bar[6]]).reshape(4, 1)
            nu[i] = np.ravel(new_z)
...
class MLPFAssociation:
    ...
    def associate(self, measurements):
        ...
        # Difference
        nu = z_hat.reshape(-1, 1, 4) - measurements.reshape(1, -1, 4)
